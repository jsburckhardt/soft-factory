...
SET CREATED_ADRS := <ADRS> (from "Agent Inference" using RESEARCH_BRIEF, ADR_TEMPLATE)
SET CREATED_CORE_COMPONENTS := <CORE_COMPONENTS> (from "Agent Inference" using RESEARCH_BRIEF, CORE_COMPONENT_TEMPLATE)
IF CREATED_ADRS is not empty OR CREATED_CORE_COMPONENTS is not empty:
  SET UPDATED_DECISION_LOG := <LOG> (from "Agent Inference" using DECISION_LOG, CREATED_ADRS, CREATED_CORE_COMPONENTS)
  USE `Write` where: content=UPDATED_DECISION_LOG, path=DECISION_LOG_PATH
</process>

<process id="write-action-plan" name="Write action plan">
SET ACTION_PLAN := <PLAN> (from "Agent Inference" using RESEARCH_BRIEF, CREATED_ADRS, CREATED_CORE_COMPONENTS)
...
SET TASK_BREAKDOWN := <BREAKDOWN> (from "Agent Inference" using TASKS)
...
SET TEST_PLAN := <PLAN> (from "Agent Inference" using TESTS)
